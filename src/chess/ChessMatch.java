...
public class ChessMatch {
	...
	public boolean[][] possibleMooves(ChessPosition sourcePosition) {
	...
	private boolean testCheckMate(Color color) {
		...
		for(Piece p : list) {
			...
			for(int i=0; i<board.getRows();i++) {
				for(int j=0; j<board.getColumns();j++) {
					if(mat[i][j]) {
						Position source = ((ChessPiece)p).getChessPosition().toString();
	...
	private void initialSetup() {
		...
		placeNewPiece('e', 1, new King(board, Color.WHITE, null));
		 ...
		 * new King(board, Color.WHITE, null));
		 * 
		 ...
		 * new King(board, Color.BLACK, null));
		 */
